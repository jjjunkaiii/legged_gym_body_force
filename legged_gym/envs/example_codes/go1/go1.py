...
from legged_gym.envs.go1.cpg import CPG_RL
...
class Go1(BaseTask):
    ...
    def post_physics_step(self):
        ...
        self.last_base_pos[:] = self.root_states[:, :3]
        self.last_base_quat[:] = self.root_states[:, 3:7]

        if self.cfg.domain_rand.randomize_decimation:
            self.decimation = random.randint(self.cfg.domain_rand.decimation_range[0],
                                             self.cfg.domain_rand.decimation_range[1])
    ...
    def _reset_dofs(self, env_ids):
        ...
        if self.cfg.domain_rand.randomize_reset_dof_pos:
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.domain_rand.randomize_gravity and (
                self.common_step_counter % self.cfg.domain_rand.gravity_rand_interval == 0):
    ...
    def _randomize_rigid_body_props(self, env_ids):
        ...
        if self.cfg.domain_rand.randomize_com_displacement:
            min_com_displacement_x, max_com_displacement_x = self.cfg.domain_rand.com_displacement_range_x
            self.com_displacements[env_ids, 0] = torch.rand(len(env_ids), dtype=torch.float, device=self.device,
                                                            requires_grad=False) * (
                                                         max_com_displacement_x - min_com_displacement_x) + min_com_displacement_x
            min_com_displacement_y, max_com_displacement_y = self.cfg.domain_rand.com_displacement_range_y
            self.com_displacements[env_ids, 1] = torch.rand(len(env_ids), dtype=torch.float, device=self.device,
                                                            requires_grad=False) * (
                                                         max_com_displacement_y - min_com_displacement_y) + min_com_displacement_y
            min_com_displacement_z, max_com_displacement_z = self.cfg.domain_rand.com_displacement_range_z
            self.com_displacements[env_ids, 2] = torch.rand(len(env_ids), dtype=torch.float, device=self.device,
                                                            requires_grad=False) * (
                                                         max_com_displacement_z - min_com_displacement_z) + min_com_displacement_z
    ...
    def _get_noise_scale_vec(self):
        ...
        noise_vec[36:64] = 0.  # cpg
        noise_vec[64:76] = 0  # last actions
        return noise_vec
    ...
    def _init_buffers(self):
        ...
        self.decimation = self.cfg.control.decimation if not self.cfg.domain_rand.randomize_decimation else random.randint(
            self.cfg.domain_rand.decimation_range[0], self.cfg.domain_rand.decimation_range[1])
        ...
        self.torques = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float, device=self.device,
                                   requires_grad=False)
        self.dof_des_pos = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float, device=self.device,
                                       requires_grad=False)
        ...
        self.max_vel = torch.ones(self.num_envs, dtype=torch.float, device=self.device,
                                  requires_grad=False) * self.cfg.commands.max_vel_x
        self.high_boundry_vel = torch.ones(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False) * \
                                self.command_ranges["lin_vel_x"][1]
        self.joint_pos_target = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float,
                                            device=self.device,
                                            requires_grad=False)
        self.frequency_high = torch.ones(self.num_envs, 4, dtype=torch.float, device=self.device,
                                         requires_grad=False) * self.cfg.control.freq_max
        self.frequency_low = torch.ones(self.num_envs, 4, dtype=torch.float, device=self.device,
                                        requires_grad=False) * self.cfg.control.freq_low
        ...
        self._cpg = CPG_RL(time_step=self.sim_params.dt, num_envs=self.num_envs, device=self.device,
    ...
    def _init_custom_buffers_(self):
