...
class Go2ArxRoughCfg( LeggedRobotCfg ):
    ...
    class goal_ee: #DIFF
        ...
        class ranges:
            ...
            init_pos_y = [-1 * np.pi / 3, 1 * np.pi / 3]
            final_delta_orn = [[-0, 0], [-0, 0], [-0, 0]]

        class init_ranges:
            pos_l = [0.3, 0.5] # min max [m/s]
            pos_p = [np.pi / 3, 3 * np.pi / 3]   # min max [m/s]
    ...
    class asset( LeggedRobotCfg.asset ):
        file = '{LEGGED_GYM_ROOT_DIR}/resources/robots/go2_arx/urdf/go2_arx.urdf'
        name = "go2_arx"
        foot_name = "foot"
        arm_link_name = ["arm"]
        ...
        penalize_contacts_on = ["thigh", "calf", "base"]# penalize contacts with base
